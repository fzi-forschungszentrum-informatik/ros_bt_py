        ...
        var rows = nodedata_list.map(data => {
            return (
                <tr key={title + data.key}>
                    ...
                    <td title={prettyprint_type(data.serialized_value)}
            ...
            <div className={border}
                ...
                <div className="d-flex justify-content-between">
                    <div className="d-flex minw0">
                        <h4 title={this.props.node.node_class}
                            className="node_class text-truncate">{this.props.node.node_class}</h4>
                ...
                <h5 title={this.props.node.module}
                    className="node_module text-truncate text-muted">{this.props.node.module}</h5>
                <div>{
...
class PackageLoader extends Component {
    ...
    componentDidMount() {
        this.props.getNodes('');
        this.props.getCapabilityInterfaces();
    }
            ...
            package_loader = (
                <div className="form-group">
                    ...
                    <button id="load_package"
                            className="btn btn-block btn-primary mt-2"
                            onClick={() => {
                                this.props.getNodes(this.state.package_name)
                                this.props.getCapabilityInterfaces();
                            }}>
                        Load package
                    </button>
        ...
        return (
            <div className="border rounded mb-2">
                <div onClick={this.toggleCollapsed.bind(this)}
                     className="text-center cursor-pointer font-weight-bold m-2">Package Loader <i
                    class={collapsible_icon}></i></div>
                {package_loader}
            </div>
        )
    };
        ...
        return (
            <div className="available-nodes m-1">
                <div className="vertical_list">
                    <div className="border rounded mb-2">
                        <div onClick={this.toggleNodeListCollapsed.bind(this)}
                             className="text-center cursor-pointer font-weight-bold m-2">Node List <i
                            class={node_list_collapsible_icon}></i></div>
                        {items}
                    </div>
                </div>
            </div>
...
class CapabilityImplementationItem extends Component {
    constructor(props) {
        super(props);
    }
        ...
        return (
            <div className={border}>
                <div className="d-flex justify-content-lg-center">
                    <div className="d-flex flex-column minw0">
                        <p className="text-truncate">Robot: {this.props.capabilityImplementation.robot_name}</p>
                        <p className="text-truncate">Version: {this.props.capabilityImplementation.version}</p>
                    </div>
                </div>
...
/**
 * React component for a capability interface.
 *
 * @author David Oberacker <oberacker@fzi.de>
 */
class CapabilityInterfaceItem extends Component {
    constructor(props) {
        super(props);
        this.state = {
            capability_item_collapsed: this.props.collapsed,
            capability_implementation_list_collapsed: this.props.capabilityImplementationListCollapsed,
        };
    }
    ...
    /**
     * Toggles the collapsed state of the item itself.
     * If untoggled the item should be hidden, otherwise it should be shown.
     * @param event The event that triggered the state change.
     */
    toggleCapabilityItemCollapsed(event) {
        this.setState({capability_item_collapsed: !this.state.capability_item_collapsed});
        event.stopPropagation();
    }

    /**
     * Toggles the collapsed state of the implementations associated with this interface.
     * If untoggled the item should be hidden, otherwise it should be shown.
     * @param event The event that triggered the state change.
     */
    toggleCapabilityImplementationListCollapsed(event) {
        this.setState({capability_implementation_list_collapsed: !this.state.capability_implementation_list_collapsed});
        event.stopPropagation();
    }

    /**
     * Renders the IO table for the capability interface.
     *
     * @param capability_interface_data_list The NodeData to render.
     * @param title The title of the section.
     * @returns {JSX.Element|null} Null if the list is empty or a JSX element with the representation.
     */
    renderIOTable(capability_interface_data_list, title) {
        // If there are no items in the list, don't generate any DOM
        // elements.
        if (capability_interface_data_list.length === 0) {
            return null;
        }
        let rows = capability_interface_data_list.map(data => {
            return (
                <tr key={title + data.key}>
                    <td title={data.key} className="io_key text-truncate maxw0">{data.key}</td>
                    <td title={prettyprint_type(data.serialized_value)}
    ...
    render() {
        let implementations = this.props.capabilityImplemenations
            .map((implementation) => {
                return (<CapabilityImplementationItem capabilityInterface={this.props.capabilityInterface}
        ...
        let io_table = null;
        if (!this.state.capability_item_collapsed) {
            io_table = (
                <div className="list-group">
                    {this.renderIOTable(this.props.capabilityInterface.inputs, 'Inputs')}
                    {this.renderIOTable(this.props.capabilityInterface.outputs, 'Outputs')}
                </div>
            );
        }

        let implementations_div = null;
        if (!this.state.capability_item_collapsed) {
            implementations_div = (
                <div className="d-flex flex-row minw0 available-capabilities m-1 w-100">
                    <div className="vertical_list w-100">
                        <div className="border rounded mb-2 mw-100">
                            <div onClick={this.toggleCapabilityImplementationListCollapsed.bind(this)}
                                 className="text-center cursor-pointer font-weight-bold m-2">Implementations<i
                                className={collapsible_list_icon}></i>
                            </div>
                            {implementations}
                        </div>
                    </div>
        ...
        let collapsible_list_icon = "cursor-pointer fas fa-angle-up";
        if (this.state.capability_implementation_list_collapsed) {
            collapsible_list_icon = "cursor-pointer fas fa-angle-down";
            implementations = null;
        }

        let border = "border rounded p-2 mb-2 ml-2 mr-2";
        if (this.props.highlighted) {
            border = "border rounded border-primary p-2 mb-2 ml-2 mr-2";
        }
        ...
        return (
            <div className={border}
                ...
                <div className="d-flex flex-row justify-content-between">
                    <div className="d-flex minw0">
                        <h4 title={this.props.capabilityInterface.name}
                            className="node_class text-truncate">{this.props.capabilityInterface.name}</h4>
                        <i title={this.props.capabilityInterface.description}
                           class="fas fa-question-circle pl-2 pr-2"></i>
                    </div>
                    <div className="d-flex minw0">
                        <i onClick={this.toggleCapabilityItemCollapsed.bind(this)} class={collapsible_icon}></i>
                    </div>
                </div>
                {io_table}
                {implementations_div}
...
/**
 * React component to display the list of Capability Interfaces available.
 *
 * @author David Oberacker <oberacker@fzi.de>
 */
class CapabilityInterfaceList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            capabilityInterfaceListCollapsed: props.capabilityInterfaceListCollapsed,
        };
    }
    ...
    /**
     * Event handler for the toggle collapse functionality.
     * @param event
     */
    toggleCapabilityInterfaceListCollapsed(event) {
        this.setState({capabilityInterfaceListCollapsed: !this.state.capabilityInterfaceListCollapsed});
        event.stopPropagation();
    }
    ...
    render() {
        ...
        const moduleThenName = function (a, b) {
            if (a.module < b.module) {
                return -1;
            } else if (a.module > b.module) {
                return 1;
        ...
        let capability_interfaces = this.props.availableCapabilityInterfaces.sort(byName);
        if (this.props.filteredCapabilityInterfaces && this.props.filteredCapabilityInterfaces.length > 0) {
            capability_interfaces = this.props.filteredCapabilityInterfaces;
        }

        let items = capability_interfaces
            .map((capability) => {
                function filter_by_uuid(implementation) {
                    return capability.uuid === implementation.capability_uuid
                }

                let available_implementations = this.props.availableCapabilityImplementations.filter(filter_by_uuid);

                let highlighted = false;
                if (this.props.draggingCapabilityInterfaceListItem
                    && this.props.draggingCapabilityInterfaceListItem.module === capability.module
                    && this.props.draggingCapabilityInterfaceListItem.node_class === capability.node_class) {
                    highlighted = true;
                }
                return (<CapabilityInterfaceItem capabilityInterface={capability}
                                                 capabilityImplemenations={available_implementations}
                                                 collapsed={true}
                                                 toggleCapabilityImplementationListCollapsed={true}
                                                 highlighted={false}
                                                 onDragging={this.props.onCapabilityInterfaceDragging}
                />);
            });

        let capability_list_collapsible_icon = "fas fa-angle-up";

        if (this.state.capabilityInterfaceListCollapsed) {
            capability_list_collapsible_icon = "fas fa-angle-down";
            items = null;
        }

        return (
            <div className="available-capabilities m-1">
                <div className="vertical_list">
                    <div className="border rounded mb-2">
                        <div onClick={this.toggleCapabilityInterfaceListCollapsed.bind(this)}
                             className="text-center cursor-pointer font-weight-bold m-2">Capability Interfaces List <i
                            class={capability_list_collapsible_icon}></i></div>
                        {items}
                    </div>
                </div>
            </div>
    ...
    render() {
        return (
            <Fragment>
                ...
                <div>
                    <input ref={this.fileref} type="file" style={{display: "none"}}
...
class D3BehaviorTreeEditor extends Component {
    ...
    layoutNodes(svg, width, height, root) {
        ...
        link.transition()
            .duration(250).attr("d", d3.linkVertical()
            .source(function (d) {
        ...
        return (
            <div className="d-flex flex-column">
                <button className="btn btn-block btn-primary"
                        ...
                        onClick={this.onClickAdd}>Add to Tree
                </button>
    ...
    renderPackageSearchResults(results) {
        if (results.length > 0) {
            var result_rows = results.map((x, i) => {
                return (
                    <div className="list-group-item search-result align-items-start"
...
class MultipleSelection extends Component {
    ...
    buildNodeMessage() {
        ...
        return {
            ...
            options: [{
                key: 'capability',
                serialized_value: JSON.stringify({
                    "py/object": "bt_capabilities.nodes.capability.CapabilityType",
            ...
            },
                {
                    key: 'implementation',
                    serialized_value: JSON.stringify({
                        "py/object": "bt_capabilities.nodes.capability.ImplementationType",
    ...
    renderCapabilitySearchResults(results) {
        if (results.length > 0) {
            ...
            var result_rows = unique_results.map(x => {
                return (
                    <div className="list-group-item search-result align-items-start"
                         onClick={() => this.selectCapabilitySearchResult(x.capability)}>
                        <div className="d-flex w-100 justify-content-between">
        ...
        if (this.props.cm_available) {
            return (
                <div className="d-flex flex-column">
                    ...
                    <div className="d-flex flex-column">
                        ...
                        <h5>Package <i title="The ROS package in which the newly created subtree will be saved in"
                        ...
                        <h5>Target <i title="The target robot, leave empty if this is a capability for generic robots"
                                      class="fas fa-question-circle"></i></h5>
                        <input className="form-control-lg mb-2"
                               type="text"
            ...
            return (
                <div className="d-flex flex-column">
                    ...
                    <div className="d-flex flex-column">
                        <h5>Filename <i title="Filename/Path of the subtree TODO" class="fas fa-question-circle"></i>
                        </h5>
                        <input className="form-control-lg mb-2"
                               type="text"
...
class SelectedNode extends Component {
    ...
    nodeClassChangeHandler(event) {
        ...
        if (flowControlNode && flowControlNode.length == 1) {
            ...
            this.setState({
                node_class: flowControlNode.node_class,
                ...
                <div className="form-group">
                    <label className="d-block">{paramItem.key}
                        <CapabilityTypeInput capability={paramItem.value.value}
                                             ros={this.props.ros}
                                             bt_namespace={this.props.bt_namespace}
                                             available_capability_interfaces={this.props.available_capability_interfaces}
                                             onValidityChange={onValidityChange}
                                             onFocus={this.onFocus}
                                             onNewValue={onNewValue}
            ...
            <div className="d-flex flex-column">
                <div className="btn-group d-flex mb-2" role="group">
                    <button className="btn btn-danger w-100"
                            onClick={this.onClickDelete}>Delete Edge
                    </button>
                </div>
                <div className="row">
                    <div className="col">
...
class CapabilityTypeInput extends Component {
    constructor(props) {
        ...
        for (let index = 0; index < props.available_capability_interfaces.length; index++) {
            if (!capabilities.has(props.available_capability_interfaces[index].capability)) {
                capability_objects.push(props.available_capability_interfaces[index]);
            }
            capabilities.add(props.available_capability_interfaces[index].capability);
        }
...
class ImplementationTypeInput extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevProps.capability !== this.props.capability) {
            for (let index = 0; index < this.props.available_capability_interfaces.length; index++) {
                if (this.props.available_capability_interfaces[index].capability === this.props.capability) {
                    if (!this.implementations.has(this.props.available_capability_interfaces[index].implementation)) {
                        this.implementation_objects.push(this.props.available_capability_interfaces[index]);
                    }
                    this.implementations.add(this.props.available_capability_interfaces[index].implementation);
                }
            }
    ...
    renderCapabilitySearchResults(results) {
        if (results.length > 0) {
            var result_rows = results.map(x => {
                return (
                    <div className="list-group-item search-result align-items-start"
                         onClick={() => this.selectCapabilitySearchResult(x.implementation)}>
                        <div className="d-flex w-100 justify-content-between">
